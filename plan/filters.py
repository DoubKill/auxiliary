...
class PalletFeedbacksFilter(django_filters.rest_framework.FilterSet):
    ...
    product_no = django_filters.CharFilter(field_name='product_day_plan__product_batching__stage_product_batch_no',
                                           help_text='胶料编码')
    begin_time = django_filters.DateTimeFilter(field_name='created_date', lookup_expr="gte",
                                               help_text='开始时间')
    end_time = django_filters.DateTimeFilter(field_name='created_date', lookup_expr="lte",
                                             help_text='结束时间')
