...
class PlanReceiveSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not product_batching:
            raise serializers.ValidationError('该胶料配料标准{}在群控上没有对应的机台配方'.format(attrs.get('product_batching')))
        ...
        if not pb_obj:
            raise serializers.ValidationError(
                '该胶料配料标准{}在群控上没有同名的机台配方'.format(pdp_dict['product_batching']['stage_product_batch_no']))
        # 校验胶料日计划设备
